...
{
	var showgraph="select ((sum(duration * percent_complete/100))/(sum(duration))) *100 as project_status from task where Project_Name = 'CMPE281'";
	console.log("Query is:"+showgraph);
	...
	mysql.fetchData(function(err,results){
		...
		{
			res.send({"statusData":JSON.stringify(results)});
		}
	},showgraph);
}
